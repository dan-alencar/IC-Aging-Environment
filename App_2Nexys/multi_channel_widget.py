...
class MultiChannelPanel(QWidget):
    ...
    def __init__(self, num_channels, parent=None, plot_window_size=200, key_prefix='dut'):
        super().__init__(parent)
        self.num_channels = num_channels
        # key_prefix lets App_2Nexys reuse this widget once per physical
        # DUT (key_prefix='dut0'/'dut1') instead of once per app.
    ...
    def update_plot_data(self, data_row):
        ...
        for i, p in enumerate(self.channel_plots):
            slack = data_row.get(f'{self.key_prefix}_slack_ch{i}', 0)
            alarm = bool(data_row.get(f'{self.key_prefix}_alarm_ch{i}', 0))
