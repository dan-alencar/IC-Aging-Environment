...
class SensorCell(QWidget):
    ...
    Shows green when inactive (0), red when active (1).
    """
...
class SensorVisualizationWidget(QWidget):
    """
    Main widget combining both sensor registers with summary statistics.
    """
    ...
    def _setup_ui(self):
        ...
        # Two register displays side by side
        registers_layout = QHBoxLayout()
        
        # AM Sensor (alarm_f)
        self.alarm_f_widget = SensorRegisterWidget("ALARM_F (AM Sensor)")
        registers_layout.addWidget(self.alarm_f_widget)
        
        # Separator
        sep = QFrame()
        sep.setFrameShape(QFrame.VLine)
        sep.setFrameShadow(QFrame.Sunken)
        registers_layout.addWidget(sep)
        
        # LF Sensor (alarm_rf)
        self.alarm_rf_widget = SensorRegisterWidget("ALARM_RF (LF Sensor)")
        registers_layout.addWidget(self.alarm_rf_widget)
        
        layout.addLayout(registers_layout)
    ...
    def updateSensorData(self, alarm_f: int, alarm_rf: int):
        """Update both sensor registers with new values."""
