...
"""
=============================================================================
TCC - Análise Comparativa: PID vs SIMC
=============================================================================
Script para análise estatística detalhada comparando o desempenho do
controlador PID desenvolvido com o controle termostático (SIMC).
...
def calculate_switching_metrics(data):
    ...
    # Para SIMC: transições 0->100 ou 100->0
...
def generate_comparison_report(pid_file, bangbang_file, setpoint, output_dir='./'):
    ...
    Args:
        ...
        bangbang_file: Caminho para CSV do teste SIMC
        setpoint: Temperatura alvo (deve ser igual nos dois testes)
        output_dir: Diretório para salvar resultados
    """
    print("=" * 70)
    print("ANÁLISE COMPARATIVA: PID vs SIMC")
    print("=" * 70)
    ...
    print(f"  SIMC: {len(bb_data)} amostras, {bb_data['time_sec'].iloc[-1]:.0f}s")
    ...
    print(f"\n{'Métrica':<35} {'PID':>15} {'SIMC':>15} {'Melhoria':>12}")
    print("-" * 77)
    ...
    if pid_metrics['jedec_compliant'] and not bb_metrics['jedec_compliant']:
        ...
        print(f"  ✗ Sistema SIMC NÃO ATENDE à norma JEDEC")
    ...
    fig.suptitle(f'Análise Comparativa: PID vs SIMC (Setpoint = {setpoint}°C)', 
                 fontsize=16, fontweight='bold')
    
    # 1. Temperatura ao longo do tempo
    ax1 = axes[0, 0]
    ax1.plot(pid_data['time_sec']/60, pid_data['oven_temp_c'], 'b-', label='PID', alpha=0.8)
    ax1.plot(bb_data['time_sec']/60, bb_data['oven_temp_c'], 'r-', label='SIMC', alpha=0.8)
    ...
    ax2.plot(bb_ss['time_sec']/60, bb_ss['oven_temp_c'], 'r-', label='SIMC', alpha=0.8)
    ...
    ax3.plot(bb_data['time_sec']/60, bb_data['oven_output_pct'], 'r-', label='SIMC', alpha=0.8)
    ...
    ax4.hist(bb_ss['oven_temp_c'], bins=30, alpha=0.7, label=f'SIMC (σ={bb_metrics["temp_std"]:.2f}°C)', color='red', density=True)
    ...
    ax5.plot(bb_data['time_sec']/60, bb_error, 'r-', label='SIMC', alpha=0.8)
    ...
    bp = ax6.boxplot(box_data, labels=['PID', 'SIMC'], patch_artist=True)
    ...
    # Salva figura
    plot_path = os.path.join(output_dir, 'comparativo_pid_simc.png')
...
\\begin{{table}}[htbp]
    ...
    \\label{{tab:comparativo-pid-SIMC}}
    \\begin{{tabular}}{{lccc}}
        \\toprule
        \\textbf{{Métrica}} & \\textbf{{SIMC}} & \\textbf{{PID}} & \\textbf{{Melhoria}} \\\\
        \\midrule
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Uso: python analise_comparativa.py <arquivo_pid.csv> <arquivo_simc.csv> <setpoint>")
        print("\nExemplo:")
        print("  python analise_comparativa.py teste_pid_100c.csv teste_simc_100c.csv 100")
        sys.exit(1)
